...
export default function TourFilters() {
  ...
  const reset = () => { setDest(''); setStyle(''); setDur(''); router.push('/tours') }

  const selectClass = `
    w-full border border-gray-200 rounded-sm px-3 py-2.5 text-[0.9rem]
    text-charcoal bg-white focus:outline-none focus:border-green
  ...
  return (
    <div className="bg-white rounded-sm shadow-card p-6 sticky top-24">
      <h3 className="font-serif text-[1.2rem] text-charcoal mb-5">Filter Safaris</h3>

      <div className="space-y-4">
        <div>
          <label className={labelClass}>Destination</label>
          <select className={selectClass} value={dest} onChange={e => setDest(e.target.value)}>
            <option value="">All Destinations</option>
            {['Kenya','Tanzania','Rwanda','Uganda','Botswana','Namibia'].map(d=>(
        ...
        <div>
          ...
          <select className={selectClass} value={dur} onChange={e => setDur(e.target.value)}>
      ...
      <button onClick={apply} className="btn btn-primary w-full text-center mt-5">
        Apply Filters
      </button>
