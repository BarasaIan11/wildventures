...
const tourLinks   = [
  { href:'/tours/great-migration-serengeti-masai-mara', label:'Great Migration Safari' },
  { href:'/tours/rwanda-gorilla-trekking', label:'Gorilla Trekking Rwanda' },
  { href:'/tours/amboseli-tsavo-classic-safari', label:'Amboseli & Tsavo' },
  { href:'/tours/uganda-primates-pearl-of-africa', label:'Uganda Primates' },
  { href:'/tours', label:'View All Tours →' },
]
const destLinks   = ['Kenya','Tanzania','Rwanda','Uganda','Botswana','Namibia']
const companyLinks = [
      ...
      <div className="px-[5%] pt-16 pb-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12">
          ...
          <div className="lg:col-span-1">
            <p className="font-serif text-[1.9rem] text-white mb-3 font-light">WildVentures</p>
            <p className="text-[0.87rem] leading-[1.75] mb-6">
              Crafting extraordinary safari experiences across East and Southern Africa since 2009.
          ...
          {/* Tours */}
          <div>
            <p className="text-white text-[0.82rem] tracking-[0.08em] uppercase font-medium mb-4">Popular Tours</p>
            <ul className="space-y-2.5">
              {tourLinks.map(({ href, label }) => (
                <li key={href}>
                  <Link href={href} className="text-[0.87rem] text-white/50 hover:text-orange transition-colors">
                    {label}
                  </Link>
                </li>
              ))}
          ...
          <div>
            <p className="text-white text-[0.82rem] tracking-[0.08em] uppercase font-medium mb-4">Destinations</p>
            <ul className="space-y-2.5 mb-7">
              {destLinks.map(d => (
                <li key={d}>
                  <Link href={`/destinations/${d.toLowerCase()}`} className="text-[0.87rem] text-white/50 hover:text-orange transition-colors">
                    {d}
                  </Link>
                </li>
              ))}
            </ul>
            <p className="text-white text-[0.82rem] tracking-[0.08em] uppercase font-medium mb-4">Company</p>
            <ul className="space-y-2.5">
          ...
          {/* Contact */}
          <div>
            <p className="text-white text-[0.82rem] tracking-[0.08em] uppercase font-medium mb-4">Get In Touch</p>
            <div className="space-y-3.5">
              {[
                { Icon: MapPin,  text: 'Westlands Business Park,\nNairobi, Kenya' },
                ...
                <div key={text} className="flex gap-3 items-start">
                  ...
                  <span className="text-[0.87rem] leading-[1.5] whitespace-pre-line">{text}</span>
                </div>
              ))}
            </div>
      ...
      <div className="border-t border-white/10 px-[5%] py-5 flex flex-col sm:flex-row justify-between items-center gap-3">
        ...
        <div className="flex gap-5 text-[0.78rem]">
          {['Privacy Policy','Terms of Use','Cookie Policy'].map(l => (
            <a key={l} href="#" className="text-white/30 hover:text-orange transition-colors">{l}</a>
          ))}
        </div>
      </div>
    </footer>
